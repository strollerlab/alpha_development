...
#   - Supplementary_Results_Fig_R1_Table_ROIs_Trajectories_**variabletype**.html
#   - Fig_SR1_GAMM_ROIs_Development_all_metrics.jpeg
...
# path2data: OSF data.
# path2tabs: MainText/Tables/Development   -> whole-brain GAMM dev. table
# path2figs: SupplementaryInformation/Results/Figures/  -> Fig SR1 (ROI GAMM trajectories)
# path2suppres: SupplementaryInformation/Results/Tables/  -> ROI-stratified GAMM tables
...
path2tabs    = file.path(path2root, "MainText",  "Tables", "Development")
path2suppres = file.path(path2root, "SupplementaryInformation", "Results", "Tables")   
path2figs    = file.path(path2root, "SupplementaryInformation", "Results", "Figures")  
...
# prop_epochs is z-scored.
# inclusion_lmm = 1 marks children included in the analysis. 
...
burst_data_regional = read_csv(file.path(path2data, "BurstProperties_ByCycle_Long.csv")) |>
  ...
  pivot_wider(names_from = is_burst,
              values_from = c(volt_amp, band_amp, frequency,
                               prop_bursty_cycles_burst, prop_bursty_epochs, avg_burst_duration)) |>
  # Drop NoBurst proportions (undefined ratios; keep only Burst-exclusive metrics because these are duplicated)
  dplyr::select(-c(prop_bursty_cycles_burst_NoBurst, prop_bursty_epochs_NoBurst,
                   avg_burst_duration_NoBurst)) |>
  # Recode the variables for consistency.
  ...
  # Compute amplitude correction factors (Burst / NoBurst ratios) to normalize burst energy by cycle type (used in ROI analysis)
  mutate(
...
# =============================================================================
# Fits GAMM: pow ~ s(age_months, k=4/3) + region + prop_epochs + Cohort +
#                  GestationalAge_weeks [+ model_fit if Specparam metric]. Note: offset (K = 3)
...
for (item in metrics_config) {
  ...
  if (n_unique_ids > 1 && nrow(model_data) > 2 * n_unique_ids) {
    ...
    if (f == "offset") { # Offset with K > 3 produced negative r2, indicating poor model.
...
for (f in inter_voi) {
  ...
  if (f == "volt_amp") {flabel = "Burst Peak-to-Peak Amp."} else {flabel = "Burst Band Amp."} 
  stable = 'burst'
...
if (length(results_list_main) > 0) {
  ...
  for (t in unique(results_list_main$stable)) {
    ...
    save_as_html(path = file.path(path2tabs, paste0("Complementary_Fig_2_3_Table_WholeBrain_GAMM_Developtment_", t, ".html")))
  }
}
...
if (length(results_list_inter) > 0) {
  ...
  bind_rows(results_list_inter) |>
    ...
    padding(padding = 1, part = "all") |>
    flextable::font(fontname = "Arial", part = "all") |> fontsize(size = 10, part = "all") |>
    save_as_html(path = file.path(path2tabs, "Complementary_Fig_2_3_Table_WholeBrain_GAMM_Developtment_BurstInteraction.html"))
}
...
for (item in metrics_config_reg) {
  ...
  if (nrow(model_data) > 100) {
    ...
    # We do not order the data because we are interested in the individual developmental slopes rather than compare them to the central region (ref.).
    if ( f == "offset") { # Offset with K > 3 produced negative r2, indicating a poor model.
    ...
    # CI of derivative not crossing 0 indicates significant developmental change. 
    ...
    reg_plot <- ggplot(pred_grid, aes(x = age_months, group = region)) +
      ...
      geom_line(aes(y = sig_y_plot, color = region), linewidth = 2.5, na.rm = TRUE) + # geom_line was more stable geom_ribbon.
      labs(x = "Age (months)", 
           y = plabel,
...
if (length(results_list_region) > 0) {
  ...
  for (t in unique(results_list_region$stable)) {
    ...
    theme_booktabs() |> autofit() |>
      ...
      padding(padding = 1, part = "all") |>
      flextable::font(fontname = "Arial", part = "all") |> fontsize(size = 10, part = "all") |>
    save_as_html(path = file.path(path2suppres, paste0("Supplementary_Results_Fig_R1_Table_ROIs_Differences_", t, ".html")))
  }
}
...
if (length(results_list_reg_smooth) > 0) {
  ...
  results|>
    ...
    padding(padding = 1, part = "all") |>
    flextable::font(fontname = "Arial", part = "all") |> fontsize(size = 10, part = "all") |>
    save_as_html(path = file.path(path2suppres, paste0("Supplementary_Results_Fig_R1_Table_ROIs_Trajectories_", t, ".html")))
  }
}
