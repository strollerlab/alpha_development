...
# -----------------------------------------------------------------------------
# Script: DataAnalysis_4_AlphaPeakPrediction_and_MLM_models_voltamp.R
...
#   - Complementary_FigS8_Alpha_Peak_Classification_Beta_Coeffs_Table.html
#   - Complementary_FigS8_Alpha_Peak_Classification_Table.html
#.  - Complementary_FigS8_Contributions_Burst_Rhythm_to_ParametrizedPSD_**visit**.html
#   - FigS8_Contributions_Burst_Rhythm_to_ParametrizedPSD.jpeg
...
# pred_model = FALSE: skip the classification loop (fast re-run for LMM / mediation only).
pred_model   = TRUE
...
# This script is the Extended Data Fig 5voltage-amplitude robustness check -> all outputs are
# Supplementary (Extended Data Fig. 5 + companion tables). See README taxonomy.
path2save = file.path(path2root, "ExtendedData",  "Tables")  # Extended Data Fig 5 robustness tables
path2figs = file.path(path2root, "ExtendedData", "Figures")  # Extended Data Fig. 5
...
electrodes = read_csv(file.path(path2data, "electrodes.csv")) |>
  ...
  # Expand electrode region abbreviations to full region names for display/analysis
...
voi = c("alpha_LAcH", "prop_bursty_epochs", "avg_burst_duration", "volt_amp_corrected")
...
plot_ed_fig5a3 = data_plot_age1 |>
  pivot_longer(
    cols    = c(alpha_LAcH, volt_amp_corrected, avg_burst_duration, prop_bursty_epochs),
    names_to  = "variable",
    values_to = "value"
  ) |>
  ...
  mutate(variable = factor(variable,
                           levels = c("alpha_LAcH", "volt_amp_corrected", "avg_burst_duration", "prop_bursty_epochs"),
...
if (pred_model) {
  ...
  # --- ROC curve plot ---
  plot_ed_fig5a1 = proc_graph_ggplot2(model_alpha_peak$roc) +
    ggpubr::theme_pubr() + THEME_COMPACT + THEME_TEXT_COMPACT +
    labs(title = "") +
    scale_fill_manual(values = COLORS_MAIN[2])
  ...
  plot_ed_fig5a2 = ggplot(model_estimates, aes(x = term, y = beta)) +
  ...
  plot_ed_fig5_full = ggarrange(plot_ed_fig5a1, plot_ed_fig5a2, plot_ed_fig5a3, ncol = 3,
  ...
  alpha_estimates = model_alpha_peak$results |>
    mutate(term = factor(term,
                         levels = c("alpha_LAcH", "volt_amp_corrected", "avg_burst_duration", "prop_bursty_epochs"),
                         labels = c("Lifespan", "Corrected Volt. Amp.", "Burst Duration", "Prop. Epochs\nw/ Burst")
    ))|>
    ...
    flextable::font(fontname = "Arial", part = "all") |> fontsize(size = 10, part = "all") |>
    align(i = 1, part = "header", align = "center") |> 
    save_as_html(path = file.path(path2save, "Complementary_Extended_Data_Fig_5_Alpha_Peak_Classification_Beta_Coeffs_corrected_voltage_check.html"))
...
predictors_lmm = c("volt_amp_corrected", "avg_burst_duration",
...
for (age in unique(data_merged_region$session_age)) {
  ...
  for (f in voi_lmm) {
    fixed_formula = as.formula(paste(
      f, "~ volt_amp_corrected + avg_burst_duration + prop_bursty_epochs +
...
results_df = bind_rows(results_list_lmm) |>
  ...
  mutate(
    ...
    term_clean    = case_when(
      term == "volt_amp_corrected"  ~ "Corrected Volt. Amp.",
...
heatmat_fig_ed_5= ggplot(results_df, aes(x = factor(session_age), y = term_clean,
                                   fill = estimate)) +
  ...
  for (d in unique(results_df$Dependent_Var)) {
    ...
    tbl_data = results_df |>
      ...
      mutate(Predictor = factor(term, levels = c("volt_amp_corrected", "avg_burst_duration",
                                                 "prop_bursty_epochs", "alpha_LAcH"),
                                labels = c("Corrected Volt. Amp.", "Burst Duration",
                                           "Prop. of Epochs\nw/ Burst", "Lifespan"))) |>
      ...
      # cited as t(df) = ..., P = ... (Nature Portfolio reporting standard).
  ...
  bind_rows(supp_rows) |>
    ...
    nn_supp_table(id = "SR11", cols = c(
      ...
      note = paste0(
        "Visit-stratified linear mixed models: Alpha metric ~ Corrected Volt. Amp. + Burst duration + ",
...
plot_full = ggarrange(plot_ed_fig5_full, heatmat_fig_ed_5, labels = c("", "d"),
                      nrow = 2, heights = c(1, 0.75))
ggsave(plot = plot_full,
       filename = file.path(path2figs, "Extended_Data_Fig_5_Contributions_Burst_Rhythm_to_ParametrizedPSD_corrected_voltage_check.jpeg"),
